...
The BMS speaks KWP2000-style one-byte local identifiers (0x21 service) on the
ISO-TP pair 0x79B/0x7BB. The UDS superclass handles the ISO-TP transport, PID
scan cycle, retries and DTC sequences; this file only decodes the group
responses received via handle_pid().
...
String RenaultZoeGen1Battery::get_uds_info_html() {
  // Per-instance "more info" section. Reads only this battery's own fields,
  // so it renders correctly for battery 2/3 as well.
  String content;
  content += "<h4>CUV " + String(LB_CUV) + "</h4>";
  content += "<h4>HVBIR " + String(LB_HVBIR) + "</h4>";
  content += "<h4>HVBUV " + String(LB_HVBUV) + "</h4>";
  content += "<h4>EOCR " + String(LB_EOCR) + "</h4>";
  content += "<h4>HVBOC " + String(LB_HVBOC) + "</h4>";
  content += "<h4>HVBOT " + String(LB_HVBOT) + "</h4>";
  content += "<h4>HVBOV " + String(LB_HVBOV) + "</h4>";
  content += "<h4>COV " + String(LB_COV) + "</h4>";
  content += "<h4>Battery mileage " + String(battery_mileage_in_km) + " km</h4>";
  content += "<h4>Alltime energy " + String(kWh_from_beginning_of_battery_life) + " kWh</h4>";
  return content;
}
