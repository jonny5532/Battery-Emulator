...
void UdsCanBattery::transmit_uds_can(unsigned long currentMillis) {
  if (currentMillis - previousUdsMillis200 >= INTERVAL_200_MS) {
    ...
    if (user_request_clear_dtc) {
      if (uds_attempts_remaining > 0) {
        uds_attempts_remaining--;
      } else {
        user_request_clear_dtc = false;  // Give up after attempts are exhausted
      }
    ...
    if (user_request_read_dtc) {
      if (uds_attempts_remaining > 0) {
        ...
        user_request_read_dtc = false;  // Give up after attempts are exhausted
      }
