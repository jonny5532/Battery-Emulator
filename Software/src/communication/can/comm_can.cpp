...
receive_frame_can_native() {  // This section checks if we have a complete CAN message incoming on native CAN port
  ...
  // since the last poll (ERR_BUS_OFF is reported once per bus-off event).
  ...
  if (can_err_flags) {
    ...
    if (can_err_flags & TWAI_Lite::ERR_BUS_OFF) {
      // The controller stays offline after a bus-off until recovery is
      // started. Recovery rejoins the bus once it has seen 128 x 11
      // recessive bits; frames sent meanwhile are queued.
      logging.println("Native CAN: starting bus-off recovery");
      twai_lite.recover();
    }
  }
