...
#define STATUS_RS   0x10  // receive status (receiving a frame)
#define STATUS_TS   0x20  // transmit status (transmitting a frame)
#define STATUS_ES   0x40  // error status (TEC or REC >= EWL)
#define STATUS_BS   0x80  // bus status (bus-off)
#define STATUS_MS   0x100 // miss status (ESP32S3 only: current RX frame was overrun)

//--- Interrupt bits. Written as the whole register: this also clears the
//--- ESP32 brp_div bit (bit 4), which is never needed as BRP is kept <= 128
...
//--- Clock divider register: CLKOUT disable bit (bit 3 on ESP32, bit 8 on ESP32S3)
#ifdef CONFIG_IDF_TARGET_ESP32
  #define CDR_CLKOUT_OFF 0x08
  #define CDR_EXT_LAYOUT 0x80  // extended (PeliCAN) register layout
#else
  #define CDR_CLKOUT_OFF 0x100
  #define CDR_EXT_LAYOUT 0     // ESP32S3 is always in the extended layout
#endif

//--- The RX-frame-invalid, RX-FIFO-corrupt, bus-off-REC and lost-TX-interrupt
//--- errata only exist on the original ESP32 (ESP-IDF esp32/twai_ll.h). The
//--- ESP32S3 only has the listen-only dominant bit issue.
#ifdef CONFIG_IDF_TARGET_ESP32
  #define TWAI_LITE_ESP32_ERRATA 1
#endif
...
TWAI_Lite::TWAI_Lite()
    : _base(TWAI_BASE), _loopback(false), _listen_only(false),
      ...
      _tx_pending(false), _running(false), _paused(false), _bus_off(false), _recovering(false),
      _rx_overflow(false), _error_flags(0),
...
bool TWAI_Lite::begin(const TWAI_Lite_Speed& speed, gpio_num_t tx_pin, gpio_num_t rx_pin, bool loopback,
  ...
  // Enable the clock and reset the peripheral (it comes out of the reset in
  // reset mode, so the TX output stays recessive while the pins are routed)
  ...
  *reg(REG_MODE) = MODE_RM;
  ...
  // Drop frames left over from before the re-initialisation (e.g. queued
  // during a bus-off or at the old bit rate): they are stale, and leaving
  // them queued would let new frames overtake them. The controller is in
  // reset mode and _running is false, so the ISR no longer touches the TX
  // queue.
  xQueueReset(_tx_queue);
  xQueueReset(_rx_queue);

  // Route the GPIO pins (outside the critical section: pinMode goes through
  // the Arduino peripheral manager). Pull up RX like ESP-IDF so a missing
  // transceiver reads as a recessive (idle) bus rather than noise.
...
void TWAI_Lite::end() {
  ...
  *reg(REG_INT_ENA) = 0;      // disable all TWAI interrupts
  portEXIT_CRITICAL(&_spinlock);
  if (_isr_handle != nullptr) {
...
// Writes all configuration registers, with the given TX/RX error counters.
// Must be called in reset mode; exits reset mode at the end to start the
// controller.
void TWAI_Lite::configure(uint8_t tec, uint8_t rec) {
  // Extended register layout first: on the ESP32 the other registers are at
  // different addresses/bits in the basic layout. CLKOUT disabled.
  *reg(REG_CDR) = CDR_EXT_LAYOUT | CDR_CLKOUT_OFF;

  // Operating mode bits can only be written in reset mode: keep RM set here,
  // otherwise the controller would start before the error counters (only
  // writable in reset mode) are programmed.
  const uint32_t mode = modeBits();
  *reg(REG_MODE) = mode | MODE_RM;
  ...
  *reg(REG_TEC) = tec;
  ...
  // Setting REC to 128 forces it error passive; the counters stay frozen in
  // listen only mode (TWAI_ERRATA_FIX_LISTEN_ONLY_DOM)
  *reg(REG_REC) = _listen_only ? 128 : rec;

  // Interrupts: RX, TX, error, bus error (also clears the ESP32 brp_div bit)
  *reg(REG_INT_ENA) = INT_ENABLED;
...
// ESP32 errata: entering reset mode while a frame is on the bus corrupts the
// next received frame. Like ESP-IDF's _node_disable(), hardware reset the
// peripheral if it was busy. Must be called in reset mode; the peripheral is
// left in reset mode, and configure() must be called before restarting.
void TWAI_Lite::hwResetIfBusy() {
#ifdef TWAI_LITE_ESP32_ERRATA
  if (*reg(REG_STATUS) & (STATUS_RS | STATUS_TS)) {
    TWAI_HW_RESET();
  }
#endif
}
...
void TWAI_Lite::errataReset() {
  ...
  *reg(REG_MODE) = MODE_RM;  // enter reset mode (stops the controller)
  while (!(*reg(REG_MODE) & MODE_RM)) *reg(REG_MODE) = MODE_RM;
  // Preserve the error counters across the reset like ESP-IDF
  // (twai_ll_save_reg()/twai_ll_restore_reg()), so fault confinement
  // (error warning/passive/bus-off) keeps working while resets are frequent
  const uint8_t tec = *reg(REG_TEC) & 0xFF;
  const uint8_t rec = *reg(REG_REC) & 0xFF;
  TWAI_HW_RESET();           // resets the peripheral into reset mode
  configure(tec, rec);       // rewrite config and restart
  if (retry_tx) {
...
bool TWAI_Lite::sendFrame(const TWAI_Lite_Frame& msg) {
  // Listen only mode cannot transmit (the TX buffer would never be released)
  if (_listen_only || _tx_queue == nullptr) return false;
  portENTER_CRITICAL(&_spinlock);
  bool ok;
  if (_running && !_tx_pending && uxQueueMessagesWaiting(_tx_queue) == 0) {
    // Hardware TX buffer is free, the controller is running and nothing is
    // queued ahead of this frame: transmit now
    startTransfer(msg);
    _tx_pending = true;
    ok = true;
  } else {
    // Paused, bus-off (the controller is in reset mode, where the TX buffer
    // addresses are the acceptance filter registers) or buffer busy: queue
    // it. The frame is handed to the hardware by the ISR, by kickTx() once
    // running again, or when bus-off recovery completes.
...
void TWAI_Lite::startTransfer(const TWAI_Lite_Frame& f) {
  ...
  *reg(REG_FRAME_INFO) = (f.ext ? FRAME_EFF : 0) | (rtr ? FRAME_RTR : 0) | dlc;
  if (f.ext) {  // Extended frame: 29 bit ID + data at 0x054
    const uint32_t id = f.id & EXT_ID_MASK;
    *reg(REG_ID_0) = (id >> 21) & 0xFF;
    *reg(REG_ID_1) = (id >> 13) & 0xFF;
    *reg(REG_ID_2) = (id >> 5) & 0xFF;
    *reg(REG_ID_3) = (id << 3) & 0xFF;
    if (!rtr) {
      for (uint8_t i = 0; i < dlc; i++) *reg(DATA_EFF(i)) = f.data[i];
    }
  } else {  // Standard frame: 11 bit ID + data at 0x04C
    const uint32_t id = f.id & STD_ID_MASK;
    *reg(REG_ID_0) = (id >> 3) & 0xFF;
    *reg(REG_ID_1) = (id << 5) & 0xFF;
    if (!rtr) {
...
// Called after a reconfiguration (task context): re-send the aborted
// in-flight frame if retry is set, otherwise start the next queued frame
void TWAI_Lite::kickTx(bool retry) {
  if (retry) {
    startTransfer(_tx_current);
    _tx_pending = true;
  } else {
...
void TWAI_Lite::changeSpeed(const TWAI_Lite_Speed& new_speed) {
  ...
  if (_paused) {
    // Stay in reset mode; pause(false) applies the new timing
    portEXIT_CRITICAL(&_spinlock);
    return;
  }
  // Only re-send the in-flight frame if it had not completed yet (the abort
  // cannot stop a frame already on the bus)
  const bool retry_tx = _tx_pending && !(*reg(REG_STATUS) & STATUS_TBS);
...
void TWAI_Lite::pause(bool paused) {
  ...
  if (paused && !_paused) {
    ...
    *reg(REG_MODE) = MODE_RM;  // reset mode stops RX/TX and stops ACKing
    ...
    configure(0, 0);           // exits reset mode, sets _running = true
    kickTx(false);
    _paused = false;
  }
  portEXIT_CRITICAL(&_spinlock);
...
bool TWAI_Lite::recover() {
  portENTER_CRITICAL(&_spinlock);
  const bool start = _bus_off && !_recovering && !_paused;
  if (start) {
    // Exiting reset mode starts the bus-off recovery sequence: the controller
    // waits for 128 occurrences of 11 recessive bits, then clears BS and
    // raises EI, where the ISR resumes transmission (like ESP-IDF's
    // twai_hal_start_bus_recovery())
    _recovering = true;
    *reg(REG_MODE) = modeBits();
  }
  portEXIT_CRITICAL(&_spinlock);
  return start;
}

uint32_t TWAI_Lite::actualBitRate() const {
  const uint32_t tq = 1 + _tseg1 + _tseg2;
  return (_brp == 0) ? 0 : getApbFrequency() / (_brp * tq);
}
...
void TWAI_Lite::drainRxFifo(BaseType_t& woken) {
  while (*reg(REG_RX_MSG_CNT) > 0) {
    const uint32_t status = *reg(REG_STATUS);
#ifdef CONFIG_IDF_TARGET_ESP32
    if (status & STATUS_DOS) {
      // Data overrun: frames that did not fit in the FIFO are still counted
      // by the RX message counter but hold no valid data, and they cannot be
      // told apart from valid ones. Like ESP-IDF
      // (twai_hal_clear_rx_fifo_overrun()), discard everything left in the
      // FIFO, then clear the overrun.
      _rx_overflow = true;
      while (*reg(REG_RX_MSG_CNT) > 0) *reg(REG_CMD) |= CMD_RRB;
      *reg(REG_CMD) |= CMD_CDO;
      break;
    }
#else
    if (status & STATUS_MS) {
      // ESP32S3: this particular FIFO entry is an overrun frame with no valid
      // data. Release it and carry on with the next one (as ESP-IDF).
      _rx_overflow = true;
      *reg(REG_CMD) |= CMD_RRB;
      continue;
    }
...
// ISR: load the next queued frame into the (free) hardware TX buffer
void TWAI_Lite::startNextFromIsr(BaseType_t& woken) {
  TWAI_Lite_Frame next;
...
void TWAI_Lite::handleInterrupt() {
  ...
  // Snapshot the status together with it (like twai_hal_decode_interrupt()),
  // so that the TX and error decisions below are based on the state that
  // matches these interrupts, not on a later one (after the RX drain or an
  // errata reset).
...
#ifdef TWAI_LITE_ESP32_ERRATA
    // Errata: invalid RX frame. If a bus error occurred in the data or CRC
    // field of a received frame, the data of the next received frame can be
    // invalid. Reset the peripheral to clear the corrupted state
  ...
  if ((intr & INT_EI) && (status & STATUS_ES)) _error_flags = _error_flags | ERR_EWL;
  if ((status & STATUS_BS) && !_bus_off) {
    // Entered bus-off: the controller has put itself into reset mode and the
    // in-flight frame is lost. Stop the TX path (in reset mode the TX buffer
    // addresses are the acceptance filter registers) until recover().
    _error_flags = _error_flags | ERR_BUS_OFF;
    _bus_off = true;
    _recovering = false;
    _running = false;
    _tx_pending = false;
...
#endif
  } else if ((intr & INT_EI) && !(status & STATUS_BS) && _recovering) {
    // Bus-off recovery complete: error active again, resume transmitting
    // the frames queued meanwhile
    _bus_off = false;
    _recovering = false;
    _running = true;
    startNextFromIsr(woken);
  }
  ...
  if (!_bus_off) {
    if (reset_periph) {
      ...
      // The reset invalidates all pending RX frames and cancelled any
      // in-flight TX; errataReset() already re-sent a frame that was
      // mid-flight (then _tx_pending is true). If no frame is in flight now,
      // a TX may have completed just before the reset (its latched interrupt
      // was consumed by the INT_RAW read above) or frames are simply queued —
      // advance the TX pipeline (ESP-IDF likewise still processes its TX
      // buffer free event after a peripheral reset).
      errataReset();
      if (!_tx_pending) startNextFromIsr(woken);
    } else {
      ...
      //--- Transmit complete. The TX buffer status in the snapshot is the
      //--- authority, as in ESP-IDF: TI alone is not enough, it may be a
      //--- stale interrupt for a frame whose completion was already handled
      //--- (while the next frame is still on the bus), and on the ESP32 the
      //--- TI itself can be lost when read on the same APB cycle as it is set
      //--- (TWAI_ERRATA_FIX_TX_INTR_LOST), so a pending frame with the buffer
      //--- free counts as completed. Gated on _running so a stale TI latched
      //--- while paused cannot touch the hardware buffer.
      if (_running && _tx_pending && (status & STATUS_TBS)) {
        _tx_pending = false;
        startNextFromIsr(woken);
      }
    }
