...
Features:
 ...
 - All ESP-IDF ESP32/ESP32-S3 TWAI errata workarounds are included (all but
   the listen only one only exist on, and are only applied to, the ESP32):
...
class TWAI_Lite {
 public:
  ...
  // Non-blocking: starts bus-off recovery. After a bus-off the controller
  // stays offline (frames sent meanwhile are queued) until this is called;
  // the controller then waits for 128 occurrences of 11 recessive bits
  // before rejoining the bus, and the ISR resumes transmission. Returns false
  // if the controller is not bus-off or recovery is already in progress.
  bool recover();

  // Bitmask returned by errorFlags(): the causes that latched the error
  // status. Latched read-clear: ERR_BUS_OFF is reported once per bus-off
  // event; call recover() to bring the controller back.
  enum : uint8_t {
  ...
  // Software mirror of the hardware REC: +8 per RX-direction bus error, -1
  // per successfully received frame (0..255, EWL = 96). Unlike rec(), it is
  // not cleared by bus-off recovery, changeSpeed() or pause(); reset by
  // begin().
  ...
  // Live bus-off status bit: set from the bus-off until recover() has
  // completed. Use errorFlags() & ERR_BUS_OFF for a latched "bus-off
  // happened" signal.
  ...
  // The bit rate actually produced by the programmed timing (may differ
  // slightly from the requested one)
  uint32_t actualBitRate() const;

 private:
  // IRAM_ATTR: this accessor and the whole ISR call chain (isr, handleInterrupt,
  // drainRxFifo, startTransfer, startNextFromIsr, errataReset, configure,
  // modeBits) are placed in IRAM and
  ...
  // Operating mode register bits (without reset mode): listen only, self
  // test (loopback), single acceptance filter
  IRAM_ATTR inline uint32_t modeBits() const {
    return (_listen_only ? 0x02 : 0) | (_loopback ? 0x04 : 0) | 0x08;
  }

  static IRAM_ATTR void isr(void* arg);
  IRAM_ATTR void handleInterrupt();     // ISR body: RX drain, TX kick, errata handling
  IRAM_ATTR void drainRxFifo(BaseType_t& woken);  // Move all hardware FIFO frames into the RX queue
  IRAM_ATTR void startTransfer(const TWAI_Lite_Frame& frame);  // Load HW TX buffer + send
  IRAM_ATTR void startNextFromIsr(BaseType_t& woken);  // ISR: send the next queued frame
  void kickTx(bool retry);    // Restart TX after a reconfiguration (task context only)
  IRAM_ATTR void configure(uint8_t tec, uint8_t rec);  // Write all config registers, then exit reset mode
  IRAM_ATTR void errataReset();         // Hardware reset of the peripheral + restore config
  void hwResetIfBusy();       // ESP32 errata: HW reset after stopping mid-frame (task context)
  ...
  volatile bool _bus_off;       // Bus-off entered; TX path stopped until recovery completes
  volatile bool _recovering;    // recover() called, waiting for the controller to rejoin
